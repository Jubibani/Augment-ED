...
import com.google.ar.sceneform.samples.gltf.library.practices.quizzes.interaction_quiz.InteractionQuizResult
import com.google.ar.sceneform.samples.gltf.library.screens.PracticeActivity

class DigestiveQuizQuestion : AppCompatActivity() {
    private var userName: String? = null

    private val questionsList: ArrayList<DigestiveQuestions> = DigestiveConstants.getQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        userName = intent.getStringExtra(DigestiveConstants.USER_NAME)
        ...
        btnSubmit?.setOnClickListener {
            if (!isAnswerChecked) {
                ...
                if (currentQuestionIndex < questionsList.size - 1) {
                    ...
                    intent.putExtra(DigestiveConstants.USER_NAME, userName)
                    intent.putExtra(DigestiveConstants.TOTAL_QUESTIONS, questionsList.size)
                    intent.putExtra(DigestiveConstants.SCORE, totalScore)
                    startActivity(intent)
                    finish()
                }
