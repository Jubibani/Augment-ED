...
import com.google.ar.sceneform.samples.gltf.library.practices.quizzes.organism_quiz.OrganismQuizResult
import com.google.ar.sceneform.samples.gltf.library.screens.PracticeActivity

class InteractionQuizQuestion : AppCompatActivity() {
    private var userName: String? = null

    private val questionsList: ArrayList<InteractionQuestions> = InteractionConstants.getQuestions()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        userName = intent.getStringExtra(InteractionConstants.USER_NAME)
        ...
        btnSubmit?.setOnClickListener {
            if (!isAnswerChecked) {
                ...
                if (currentQuestionIndex < questionsList.size - 1) {
                    ...
                    val intent = Intent(this, InteractionQuizResult::class.java)
                    intent.putExtra(InteractionConstants.USER_NAME, userName)
                    intent.putExtra(InteractionConstants.TOTAL_QUESTIONS, questionsList.size)
                    intent.putExtra(InteractionConstants.SCORE, totalScore)
                    startActivity(intent)
                    finish()
                }
